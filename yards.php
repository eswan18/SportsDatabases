...
<body>
  ...
  <div class="row">
	...
	<?php
	  ...
	  $stid = oci_parse($conn, 'SELECT round(avg(yards),2) as average, rush_direction FROM plays where rush_direction is not null group by rush_direction order by rush_direction');
	  oci_execute($stid);
	  ...
	  while ($row = oci_fetch_array($stid, OCI_ASSOC)) {
	    echo "<td><a class = \"table_selector\" id = \"" . $row['RUSH_DIRECTION'] . "\" value = \"" .$row['AVERAGE']. "\">" . $row['RUSH_DIRECTION']. "</a></td>";
	    echo "<tr>";
	  }
	  echo "</table>";
	?>
...
</script><script src="js/app2.js" type="text/javascript">
</script>
</body>
</html>
